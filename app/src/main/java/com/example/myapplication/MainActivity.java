...
// bai 4 tinh khoang cach ngay , thang nam giua 2 ngay a  va b ;
//         vd  ngay a : 17/05/2010  ngay b :30/09/2019

        LocalDate firt = LocalDate.of(2010, 5, 17);
        LocalDate end = LocalDate.of(2019, 3, 30);
        Period period = Period.between(firt, end);
        int days = period.getDays();
        int month = period.getMonths();
        int years = period.getYears();

//         bai 5 Truyen vao Date  theo dang  yyyy/ MM/ dd HH:mm:ss tra ve timestamp và ngược lại

        dtStart = "2019-09-23 21:17:37";
        format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date dateformat = null;
        try {
            dateformat = format.parse(dtStart);
        } catch (ParseException e) {
            e.printStackTrace();
        }

//          chuyen  ve dang timestamp
        Timestamp ts = new Timestamp(dateformat.getTime());
//          chuyen nguoc lai  :
        dateformat = new Date(ts.getTime());


//         bai 6 :

//        lam tron den phut , su dung thu vien tren mang co san
        Date nearestMinute = DateUtils.round(dateformat, Calendar.MINUTE);
        Date nearestHours = DateUtils.round(dateformat, Calendar.HOUR);
        Date nearestDay = DateUtils.round(dateformat, Calendar.DAY_OF_MONTH);


//         tuong tu voi 2 cai con lai duoc lam tron
        ts = new Timestamp(nearestHours .getTime()) ;




//         bài 7
//        lay du lieu bai 5 dateformat  = "2019-09-23 21:17:37";

        SimpleDateFormat formatter = new SimpleDateFormat("yyyy/MM/dd  HH:mm:ss");
        String format1 = formatter.format(dateformat) ;
//        resurl : 2019/09/23 21 : 17:37

        SimpleDateFormat formatter2 = new SimpleDateFormat("MMM yyyy,dd HH:mm:ss");
         format1 = formatter2.format(dateformat) ;

//          resurl : SEP 2019,23 21:17:37
...
//                  da co thi chi get ra va add them
                hashmap.get(item.getDate()).add(item);
            } else {
                // add Bill  moi tao moi hashSet , add hashSet ,put vao hashMap
                day.add(item.getDate());
...
// bai tap 7 va 8 em khong hieu de bai a
        List<List<Bill>> baitap7 = new ArrayList<>();
        for (String date : day) {
            baitap7.add((List<Bill>) hashmap.get(date));
        }
//         bai tap 8
        homeWord8();
    }

    private void homeWord8() {
        initData();
        HashMap<String, HashSet<Bill>> hashmap = new HashMap<String, HashSet<Bill>>();
...
//             kienpv bai toan 1 , tinh tong theo 2 thuat toan
    ...
    public void initData() {
        ...
        listBill.add(new Bill(5, "kienpv", 10000, "2019/02/04"));
    }
}
